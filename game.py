...
pygame.display.set_caption("Raycura_Car_Game")
...
# Load sound files
start_page_sound = pygame.mixer.Sound("start.mp3")
...
def intro_loop():
    pygame.mixer.Sound.stop(car_sound)  # Stop car sound if it was playing
    pygame.mixer.Sound.stop(pause_sound)  # Stop pause sound if it was playing
    start_page_sound.play()
    intro=True
    while intro:
        ...
        TextSurf,TextRect=text_objects("Raycura Car Game",largetext)
...
def paused():
    global pause
    pygame.mixer.Sound.stop(car_sound)  # Stop car sound if it was playing
    pygame.mixer.Sound.stop(start_page_sound)  # Stop start page sound if it was playing
    pause_sound.play()
...
def countdown():
    pygame.mixer.Sound.stop(start_page_sound)
...
def crash():
    ...
    # Restart car sound
    pygame.mixer.Sound.stop(crash_sound)  # Stop crash sound if it was playing
    car_sound.play(-1)  # Play car sound in a loop (-1 means loop indefinitely)
    game_loop()
...
def game_loop():
    ...
    while not bumped:
        ...
        if obs_starty>display_height:
            ...
            passed=passed+1
            score=passed*10
            if int(passed)%10==0 and int(passed) != 0:
                ...
                # Restart car sound after the level message is gone
                pygame.mixer.Sound.stop(level_up_sound)
                car_sound.play(-1)  # Play car sound in a loop (-1 means loop indefinitely
